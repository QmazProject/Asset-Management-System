...
export const EditServiceTemplate = () => {
        ...
        const fetchTemplate = async () => {
            ...
            try {
                ...
                if (data) {
                    ...
                    setIsCritical(data.is_critical);
                    setServiceType(data.service_type);
                    setBasedOn(data.based_on);
                    setUnitOfMeasurement(data.unit_of_measurement || 'Days');
                    setFrequencyNumber(data.frequency_number ? String(data.frequency_number) : '');
                    setNotificationMode(data.notification_mode || 'automatic');
                    setCreatedAt(data.created_at);

                    if (data.notification_mode === 'manual') {
                        setManualNotificationNumber(data.notification_number ? String(data.notification_number) : '10');
                        setManualNotificationUnit(data.notification_unit || 'Days');
                    } else {
                        setNotificationNumber(data.notification_number ?? 10);
                        setNotificationUnit(data.notification_unit || 'Days');
                    }

                    if (data.service_template_attachments) {
                        setExistingAttachments(data.service_template_attachments);
                    }
                }
            } catch (error) {
                console.error('Error fetching template:', error);
                alert('Error loading service template');
    ...
    const handleSave = async () => {
        try {
            ...
            // Check for duplicate (Name only), excluding current ID
            ...
            const { error: templateError } = await supabase
                ...
                .update({
                    ...
                    // based_on is NOT updated as it is read-only in UI logic usually for edits, but we send it anyway or keep it same
                    // Actually user requested "Based on un editable", so we shouldn't change it if it was modified (but it's disabled so it won't change).
                    based_on: basedOn,
            ...
            // 2. Upload New Files and Insert Attachments
    ...
    // Reusing the same logic from AddServiceTemplate
    useEffect(() => {
        if (basedOn === 'Period') {
                ...
                <div className="w-full max-w-5xl bg-white shadow-sm border border-gray-200 rounded-sm h-fit flex-shrink-0">
                    <div className="p-8">
                        ...
                        <div className="mb-6 w-1/2 pr-4">
                            ...
                            <select
                                className="w-full border border-gray-300 bg-gray-100 px-3 py-2 text-sm text-gray-500 focus:outline-none cursor-not-allowed"
                                value={basedOn}
                                disabled={true}
                    ...
                    <div className="p-8">
                        ...
                        {existingAttachments.length > 0 && (
                            <div className="mb-6 space-y-2">
                                <label className="block text-xs font-medium text-gray-500">Existing attachments</label>
