...
export const EditAssetPage = ({ onClose, assetToEdit }: EditAssetPageProps) => {
    ...
    const restoredServices = location.state?.returnFromManageServices ? location.state.assignedServices : []
    // restore logic moved to state init

    const [assignedServices, setAssignedServices] = useState<any[]>(restoredServices)
    ...
    // Helper to initialize state from edit data
    useEffect(() => {
        if (assetToEdit && !location.state?.returnFromManageServices) {
            const initialImages: AssetFile[] = []

            // 1. Existing Main Image
            if (assetToEdit.image_url) {
                initialImages.push({
                    id: 'main-image',
                    url: assetToEdit.image_url,
                    name: 'Main Image',
                    type: 'image',
                    isMain: true,
                    isExisting: true
            ...
            // Additional images will be fetched and added in the existing useEffect
            setImages(initialImages)
        }
    }, [assetToEdit])

    // File state removed (replaced by images/docs)
    const [isUploading, setIsUploading] = useState(false)

    // Load related data (and fresh asset data) if editing
    useEffect(() => {
        const loadRelatedData = async () => {
            if (assetToEdit && !location.state?.returnFromManageServices) {
                ...
                try {
                    const { data: freshAsset, error: assetError } = await supabase
                        .from('assets')
                        .select('*')
                    ...
                    if (assetError) {
                        console.error('Error fetching fresh asset:', assetError)
                    } else if (freshAsset) {
                        // Update Form Data with fresh values
                        setFormData({
                            scanCode: freshAsset.scan_code || '',
                            inventoryNumber: freshAsset.inventory_number || '',
                            serialNumber: freshAsset.serial_number || '',
                            csNumber: freshAsset.cs_number || '',
                            plateNumber: freshAsset.plate_number || '',
                            engineNumber: freshAsset.engine_number || '',
                            manufacturer: freshAsset.manufacturer || '',
                            model: freshAsset.model || '',
                            assetName: freshAsset.asset_name || '',
                            condition: freshAsset.condition || '',
                            availability: freshAsset.availability || '',
                            assetGroup: freshAsset.asset_group || 'Ungrouped',
                            description: freshAsset.description || '',
                            label: freshAsset.label || '',
                            currentLocationType: freshAsset.current_location_type || 'Location',
                            currentLocation: freshAsset.current_location || '',
                            responsibleEmployee: freshAsset.responsible_employee || '',
                            owner: freshAsset.owner || '',
                            ownershipType: freshAsset.ownership_type || 'Owned',
                            vendor: freshAsset.vendor || '',
                            purchaseDate: freshAsset.purchase_date || '',
                            costCode: freshAsset.cost_code || '',
                            purchaseOrderNumber: freshAsset.purchase_order_number || '',
                            purchasePrice: freshAsset.purchase_price || '',
                            purchaseCurrency: freshAsset.purchase_currency || 'Philippine Peso',
                            warrantyExpirationDate: freshAsset.warranty_expiration_date || '',
                            replacementCost: freshAsset.replacement_cost || '',
                            notes: freshAsset.notes || ''
                        })

                        // Update main image if fresh asset has it
                        if (freshAsset.image_url) {
                            setImages(prev => {
                                const hasMain = prev.some(img => img.isMain)
                                if (!hasMain) {
                                    return [{
                                        id: 'main-image',
                                        url: freshAsset.image_url,
                                        name: 'Main Image',
                                        type: 'image',
                                        isMain: true,
                                        isExisting: true
                    ...
                    console.error('Unexpected error loading asset:', err)
                }

                // 2. Fetch services
                try {
                    console.log('Fetching services for asset:', assetToEdit.id)
                    ...
                    if (servicesData) {
                        ...
                        // Deduplicate services by creating a unique key
                        const uniqueServices = servicesData.reduce((acc: any[], service) => {
                            const isDuplicate = acc.some(s =>
                                s.service_name === service.service_name &&
                                s.next_service_date === service.next_service_date &&
                        ...
                        setAssignedServices(uniqueServices.map(s => ({
                            id: s.id,
                ...
                try {
                    ...
                    if (attachmentsData) {
                        const fetchedImages: AssetFile[] = []
                        const fetchedDocs: AssetFile[] = []

                        attachmentsData.forEach(att => {
                            const fileObj: AssetFile = {
                                id: att.id,
                                name: att.file_name,
                                url: att.file_path,
                            ...
                            if (att.file_type.startsWith('image/')) {
                                fetchedImages.push({ ...fileObj, type: 'image' })
                            } else {
                                fetchedDocs.push({ ...fileObj, type: 'document' })
                            }
                        })

                        if (fetchedImages.length > 0) {
                            setImages(prev => [...prev.filter(img => img.id === 'main-image'), ...fetchedImages])
                        }
                        setDocs(fetchedDocs)
                    }
                } catch (err) {
        ...
        loadRelatedData()
    }, [assetToEdit, location.state])

    const [formData, setFormData] = useState(restoredData || (assetToEdit ? {
        scanCode: assetToEdit.scan_code || '',
        inventoryNumber: assetToEdit.inventory_number || '',
        serialNumber: assetToEdit.serial_number || '',
        csNumber: assetToEdit.cs_number || '',
        plateNumber: assetToEdit.plate_number || '',
        engineNumber: assetToEdit.engine_number || '',
        // Asset Details
        manufacturer: assetToEdit.manufacturer || '',
        model: assetToEdit.model || '',
        assetName: assetToEdit.asset_name || '',
        condition: assetToEdit.condition || '',
        availability: assetToEdit.availability || '',
        assetGroup: assetToEdit.asset_group || 'Ungrouped',
        description: assetToEdit.description || '',
        label: assetToEdit.label || '',
        // Step 2 Fields
        currentLocationType: assetToEdit.current_location_type || 'Location',
        currentLocation: assetToEdit.current_location || '',
        responsibleEmployee: assetToEdit.responsible_employee || '',
        owner: assetToEdit.owner || '',
        // Ownership Details
        ownershipType: assetToEdit.ownership_type || 'Owned',
        vendor: assetToEdit.vendor || '',
        purchaseDate: assetToEdit.purchase_date || '',
        costCode: assetToEdit.cost_code || '',
        purchaseOrderNumber: assetToEdit.purchase_order_number || '',
        purchasePrice: assetToEdit.purchase_price || '',
        purchaseCurrency: assetToEdit.purchase_currency || 'Philippine Peso',
        warrantyExpirationDate: assetToEdit.warranty_expiration_date || '',
        replacementCost: assetToEdit.replacement_cost || '',
    ...
    const validateStep1 = () => {
        ...
        setErrors(newErrors)
        return !newErrors.scanCode && !newErrors.inventoryNumber
    }
        ...
        if (currentStep === 1) {
            ...
            if (validateStep2()) {
                ...
                try {
                    ...
                    // 2. Handle Services
                    // Strategy: Delete all existing services and re-insert current list to ensure sync
                    console.log('Starting service sync for asset:', assetId)
                    console.log('Current assignedServices count:', assignedServices.length)
                    ...
                    if (deleteError) {
                        ...
                        throw deleteError // CRITICAL: Throw error to prevent continuing with bad state
                    }

                    console.log('Successfully deleted all existing services')

                    if (assignedServices.length > 0) {
                        // Deduplicate services before inserting
                        const uniqueServices = assignedServices.reduce((acc: any[], service) => {
                            const isDuplicate = acc.some(s =>
                        ...
                        console.log('Services to insert:', uniqueServices.length, 'from', assignedServices.length)

                        const servicesToInsert = uniqueServices.map(service => ({
                            asset_id: assetId,
                        ...
                        console.log('Successfully inserted', uniqueServices.length, 'services')
                    } else {
                        console.log('No services to insert - all services removed')
                    }
        ...
        <div className="min-h-screen bg-gray-50">
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        />
                                        {errors.scanCode && (
                                            <p className="text-red-600 text-xs mt-1">Please enter scan code</p>
                                        )}
                                    </div>
                                    <div>
                                        <label className="block text-sm text-gray-700 mb-2">
                                        ...
                                        {errors.inventoryNumber && (
                                            <p className="text-red-600 text-xs mt-1">Please enter inventory number</p>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {/* Image Dropzone */}
                                        <div
                                            onClick={() => !isLoadingData && images.length < 5 && (window as any).assetImageInput?.click()}
                                            className={`border-2 border-dashed border-gray-300 rounded-lg p-8 flex flex-col items-center justify-center text-center hover:bg-gray-50 transition-colors cursor-pointer ${isLoadingData || images.length >= 5 ? 'opacity-50 pointer-events-none' : ''}`}
                                        >
                                            {isLoadingData ? (
                                                <div className="flex flex-col items-center">
                                                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-lime-600 mb-2"></div>
                                ...
                                <button
                                    onClick={() => navigate('/manage-services', {
                                        state: {
                                            ...
                                            // Pass persistence data using new structure
                                            assetId: assetToEdit?.id || location.state?.assetId,
                                            isEditMode: !!assetToEdit || location.state?.isEditMode,
                                            // Pass images and docs instead of simple vars
                                            images: images,
                                            docs: docs,
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-2px_4px_rgba(0,0,0,0.05)]">
                <div className="max-w-4xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8">
                    ...
                    >
                        {isUploading ? 'Updating...' : (currentStep === 1 ? 'Next' : 'Update asset')}
                    </button>
                </div>
